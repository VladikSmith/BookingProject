...
class APIClient:
    ...
    def update_booking(self, booking_id, booking_data):
        with allure.step('Update booking'):
            url = f'{self.base_url}{Endpoints.BOOKING_ENDPOINT}/{booking_id}'
            response = requests.put(url, json=booking_data, auth=HTTPBasicAuth(Users.USERNAME, Users.PASSWORD))
            response.raise_for_status()
        with allure.step('Assert status code'):
            assert response.status_code == 200, f'Expected status 200 but got {response.status_code}'
        return response.json()
    ...
    def partial_update_booking(self, booking_id, booking_data):
        with allure.step('Partial update booking'):
            url = f'{self.base_url}{Endpoints.BOOKING_ENDPOINT}/{booking_id}'
            response = requests.patch(url, json=booking_data, auth=HTTPBasicAuth(Users.USERNAME, Users.PASSWORD))
            response.raise_for_status()
        with allure.step('Assert status code'):
            assert response.status_code == 200, f'Expected status 200 but got {response.status_code}'
        return response.json()